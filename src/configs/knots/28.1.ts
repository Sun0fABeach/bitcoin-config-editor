...
import { patchCategoryOptions, getCategoryIndex } from '@/configs/helpers'
...
export default function () {
	...
	patchCategoryOptions(categories, 'General', (options) => ({
		...
		dbcache: {
			...options.dbcache,
			typeConstraints: {
				...options.dbcache.typeConstraints,
				max: 1048576,
			},
			description:
	...
	patchCategoryOptions(categories, 'Mining', (options) => {
		delete options.blockreservedweight
	...
	patchCategoryOptions(categories, 'ZeroMQ', (options) => ({
		...options,
	...
	const debugCategoryIndex = getCategoryIndex(categories, 'Debugging & Testing')
